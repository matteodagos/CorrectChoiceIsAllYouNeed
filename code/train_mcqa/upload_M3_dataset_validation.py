...
allowed_mmlu =  ["abstract_algebra", "anatomy", "astronomy", "college_biology", "college_chemistry", 
...
def map_mmlu_pro(row):
    choices = row["options"]
    correct_choice = row["answer_index"]
    answer = f"{chr(65+correct_choice)}. " + choices[correct_choice]
    return {"QUESTION": row["question"], "ANSWER": answer, "CHOICES": choices, "RATIONALE": ""}
...
SOURCES = [
    ...
    ("mmlu_pro"            , "TIGER-Lab/MMLU-Pro",           None,      map_mmlu_pro),
...
for short_name, hf_id, config, fn in SOURCES:
    ...
    ds = load_dataset(hf_id, config, split="validation")
    if short_name == "scienceqa_textonly":
        ...
        ds = ds.shuffle()  # shuffle the order of questions
        ds = ds.select(range(500))  # only keep first 500 rows
